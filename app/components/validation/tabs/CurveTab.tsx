      ...
      {isMismatch ? (
        <div className="flex items-start gap-3 px-5 py-4 rounded-xl bg-amber-50 border border-amber-200">
          ...
          {!referralLogged ? (
            ...
            <span className="text-xs font-semibold text-amber-700 bg-amber-100 px-2 py-1 rounded shrink-0">Referral logged</span>
          )}
        </div>
      ) : (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Contract Curve ({contract.curveName})</th>
              <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Current Approved ({contract.currentCurveName})</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map(row => {
              ...
              return (
                <tr key={row.label} className={isBad ? 'bg-amber-50/50' : ''}>
                  <td className="px-5 py-3 font-medium text-slate-700">{row.label}</td>
                  <td className="px-5 py-3 text-right font-mono text-slate-700">{row.origAvg.toFixed(4)}p</td>
                  <td className="px-5 py-3 text-right font-mono text-slate-700">{row.currAvg.toFixed(4)}p</td>
                  <td className={`px-5 py-3 text-right font-mono font-semibold ${isBad ? 'text-amber-700' : 'text-slate-500'}`}>
